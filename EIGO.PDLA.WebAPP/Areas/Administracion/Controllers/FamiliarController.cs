...
namespace EIGO.PDLA.WebAPP.Areas.Administracion.Controllers
{
    ...
    public class FamiliarController : Controller
    {
        ...
        public FamiliarController(IEntityRepository<Familiar> familiarRepository)
        {
        ...
        [HttpGet]
        public async Task<JsonResult> GetFamiliaresByFuncionario(int IdFuncionario)
        {
       
            
            var Familiares = await ((FamiliarRepository)_familiarRepository).GetByIdFuncionarioAsync(IdFuncionario);

            if (Familiares != null)
            {

                return Json(new { Familiares });
