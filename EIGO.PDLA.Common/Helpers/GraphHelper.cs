...
// TAKEN AND ADAPTED FROM https://github.com/Azure-Samples/active-directory-aspnetcore-webapp-openidconnect-v2/blob/master/5-WebApp-AuthZ/5-2-Group and adapted
namespace EIGO.PDLA.Common.Helpers;
...
public static class GraphHelper
{
    ...
    public static async Task<List<string>> GetSignedInUsersGroups(TokenValidatedContext context)
    {
        ...
        if (context != null && context.Principal != null && HasOverageOccurred(context.Principal))
        {
    ...
    public static List<string> GetUserGroupsFromSession(ISession _httpContextSession)
    {
        ...
        {
            return _httpContextSession.GetAsByteArray<List<string>>("groupClaims");
        }
    ...
    /// <summary>
    /// This method is called for User CUP scenario.
    /// The method makes calls to Microsoft Graph to fetch the properties of the authenticated user.
    /// </summary>
    /// <param name="context"></param>
    /// <returns></returns>
    public static async Task<List<string>> GetUserCUP(TokenValidatedContext context)
    {
        List<string> groupClaims = new();
        try
        {
            ...
            else if (context.SecurityToken != null)
            {
                ...
                if (!context.HttpContext.Items.ContainsKey("JwtSecurityTokenUsedToCallWebAPI"))
                {
                    ...
                    context.HttpContext.Items.Add("JwtSecurityTokenUsedToCallWebAPI", context.SecurityToken);
                }

                // The properties that we want to retrieve from MemberOf endpoint.
                string select = "id,displayName,onPremisesExtensionAttributes";
...
#pragma warning restore S1854 // Unused assignments should be removed
                try
                {
                    ...
                    user = await graphClient.Me.Request().Select(select).GetAsync().ConfigureAwait(false);
                }
                ...
                if (user != null && user.OnPremisesExtensionAttributes != null)
                {
                    var CUP = user.OnPremisesExtensionAttributes.ExtensionAttribute1;
                    context.HttpContext.Session.SetString("CUP", CUP);
                    var cookieOptions = new CookieOptions { Secure = true,HttpOnly=true, SameSite=SameSiteMode.Strict, Expires = DateTimeOffset.UtcNow.AddYears(1)};
                    context.HttpContext.Response.Cookies.Append("cup",CUP,cookieOptions);
                }
            }
        }
        catch (Exception ex)
    ...
    /// <param name="context"></param>
    /// <returns></returns>
    public static async Task<List<string>> GetUserCUP(HttpContext context)
    {
        List<string> groupClaims = new();
        try
        {
            context.Session.SetString("CUP", context.Request.Cookies["cup"]);
...
#pragma warning restore S1854 // Unused assignments should be removed
                ...
                {
                    //Request to get groups and directory roles that the user is a direct member of.
                    memberPage = await graphClient.Me.MemberOf.Request().Select(select).Expand("onPremisesExtensionAttributes").GetAsync().ConfigureAwait(false);
                }
                catch (Exception graphEx)
                {
                    var exMsg = graphEx.InnerException != null ? graphEx.InnerException.Message : graphEx.Message;
                ...
                if (memberPage?.Count > 0)
                {
                    ...
                    if (allgroups?.Count > 0 && context.Principal?.Identity is ClaimsIdentity identity && !IsAccessToken(identity))
                    {
    ...
    /// <returns></returns>
    public static bool CheckUsersGroupMembership(ClaimsPrincipal claimsPrincipal, string GroupName)
    {
        bool result = false;
        // Checks if groups claim exists in claims collection of signed-in User.
        if (claimsPrincipal.Claims.Any(x => x.Type == "groups" && x.Value == GroupName))
        {
            result = true;
        }
        return result;
